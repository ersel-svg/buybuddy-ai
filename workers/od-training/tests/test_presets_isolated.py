"""
Isolated unit tests for augmentation presets.
Does not require external dependencies like albumentations.
"""
import sys
import os

# Add the src directory to path
src_dir = os.path.join(os.path.dirname(__file__), '..', 'src', 'augmentations')
sys.path.insert(0, src_dir)

# Import only the presets module directly (it doesn't require albumentations)
exec(open(os.path.join(src_dir, 'presets.py')).read())
...
def test_all_augmentation_params_have_metadata():
    """Test all augmentations have proper metadata in categories."""
    categories = get_augmentation_categories()

    for cat_name, cat_data in categories.items():
        for aug_name, aug_data in cat_data["augmentations"].items():
            assert "name" in aug_data, f"{aug_name} should have name"
            assert "description" in aug_data, f"{aug_name} should have description"
            assert "default_prob" in aug_data, f"{aug_name} should have default_prob"
            assert "params" in aug_data, f"{aug_name} should have params"

    print("✅ TC-PRESET-014: All augmentations have metadata - PASSED")


def test_dataclass_fields_count():
    """Test AugmentationPreset has all expected fields."""
    preset = AugmentationPreset()
    fields = [f for f in preset.__dataclass_fields__]

    # Count categories
    expected_min_fields = 40  # At least 40 augmentation types

    assert len(fields) >= expected_min_fields, \
        f"AugmentationPreset should have at least {expected_min_fields} fields, got {len(fields)}"

    print(f"✅ TC-PRESET-015: AugmentationPreset has {len(fields)} fields - PASSED")


if __name__ == "__main__":
    print("\n" + "="*60)
    print("RUNNING ISOLATED PRESET UNIT TESTS")
    print("="*60 + "\n")
    ...
    test_all_augmentation_params_have_metadata()
    test_dataclass_fields_count()
